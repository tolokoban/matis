                ...
                for (outputName in output) {
                    ...
                    that.links.forEach(function (link) {
                        if (link.output == outputName) {
                            ...
                            // whole input.
                            var targetInput = link.target.post(link.input, outputValue, input);
                            if (targetInput) {
 ...
 *   exec A: 74, B: 69, C: 28
 */
Tool.prototype.post = function(inputName, value, inputs) {
    var pack;
    var full;
    var i, k;
    ...
    for (i = 0 ; i < this.mailbox.length + 1 ; i++) {
        ...
        if (typeof pack[inputName] === 'undefined') {
            ...
            // this case, _tags_ must be arrays.
            foreachTag(inputs, function(key, val) {
                if (typeof pack[key] === 'undefined') pack[key] = val;
                else if (pack[key] != val) {
                    // There already is a tag here.
                    if (Array.isArray(pack[key])) {
                        // Add new tag in the array if is not already part of it.
                        if (pack[key].indexOf(val) == -1) {
                            pack[key].push(val);
                        }
                    } else {
                        // Create an array with both tags.
                        pack[key] = [pack[key], val];
                    }
                }
            });
...
Tool.debug = function(level) {
    if (!level) {
        ...
        Tool.onPost = function(srcTool, srcOutput, dstTool, dstInput, value, inputs) {
            console.log(srcTool.name() + " (" + srcOutput + ") --> ("
                        + dstInput + ") " + dstTool.name()
                        + "  <" + inputs.$tag + ">  " + JSON.stringify(value).substr(0, 30));
        };
...
/**
 * Copy all tags of `input` to `output`. A __tag__ is an attribute starting with `$`.
 * @param input {object}
 * @param output {object}
 */
function copyTags(input, output) {
    for (var k in input) {
...
/**
 * Call `f()` for each attribute starting with `$`.
 * @param f {function} - `f(key, value)`.
 */
function foreachTag(input, f, that) {
    for (var k in input) {
        if (k.charAt(0) == '$' && k.length > 1) {
            f.call(that, k, input[k]);
        }
    }
}
