...
var Tool = function(definition) {
    ...
    // Preparing input for short linking syntax.
    this.I = {};
    definition.input.forEach(function (inputName) {
        createInputAttribute(that, inputName);
    });
    ...
    // Preparing output for short linking syntax.
    this.O = function(arg) {
        if (module.exports.isTool(arg)) {
            return that.bind(arg).O;
        } else {
            if (typeof arg.target === 'undefined' || typeof arg.name === 'undefined') {
                throw Error("[Tool.bind] Trying to link output `"
                            + definition.output[0] + '` of tool `' + that.name()
                            + '` to an invalid input!');
            }
            return that.bind(arg.target, arg.name).O;
        }
    };
    definition.output.forEach(function (outputName) {
        createOutputAttribute(that, outputName);
    });
 ...
 */
Tool.prototype.name = function(v, dontAddID) {
...
Tool.prototype.link = function(outputName, target, targetInputName) {
    ...
    {
        throw Error("[async-tools.link] Invalid target for tool `" + this.name() + "`!");
    }
    ...
    if (target.definition.input.indexOf(targetInputName) == -1) {
        ...
        throw Error("[async-tools.link] ("
                    ...
                    + "Possible inputs are: " + target.definition.input.join(', '));
    }
...
Tool.debug = function(level) {
    if (!level) {
        ...
        if (typeof level.onInput !== 'undefined') {
            Tool.onInput = level.onInput;
        }
        if (typeof level.onOutput !== 'undefined') {
            Tool.onOutput = level.onOutput;
        }
        if (typeof level.onPost !== 'undefined') {
            Tool.onPost = level.onPost;
        }
    }
};
...
/**
 * If this tool  has an output named  `foo` you can connect  it to the
 * `bar` input of a target tool by using two syntaxes:
 * ```js
 * this.link('foo', target, 'bar').link('exit`, final, 'text').link(bob).link(alice);
 * ```
 *
 * ```js
 * this.O.foo(target.I.bar).exit(final.I.text)(bob)(alice);
 * ```
 *
 * But, is this useful???
 */
function createOutputAttribute(that, outputName) {
    that.O[outputName] = function(arg) {
        if (module.exports.isTool(arg)) {
            return that.bind(outputName, arg).O;
        } else {
            if (typeof arg.target === 'undefined' || typeof arg.name === 'undefined') {
                throw Error("[Tool.bind] Trying to link output `"
                            + outputName + '` of tool `' + that.name() + '` to an invalid input!');
            }
            return that.bind(outputName, arg.target, arg.name).O;
        }
    };
}


function createInputAttribute(that, inputName) {
    that.I[inputName] = {target: that, name: inputName};
}
