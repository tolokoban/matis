...
Tool.prototype.exec = function(input, resolve, reject, absorb) {
    ...
    var processAbsorb = function(output) {
    ...
    var processResolve = function(output) {
        token.flush(function() {
            ...
            if (that.links.length < 1) {
                ...
                // Resolution can  only occur with tools  that have no
                // link.    Other   tools    will   _propagate_    the
                // resolution.  But sometimes,  for tools  with links,
                // not  all  output's   attributes  are  connected  to
                // another tool. In  this case, `hasBeenPropagated` is
                // false and we must call `absorb()`.
    ...
    try {
        this.definition.exec.call(this, input, processResolve, reject, processAbsorb);
    }
    catch (ex) {
        console.error(ex);
        reject(ex);
